...
$conf{'thanks'} = '../r_download.html';

#08.設置者に届くメールの件名
$conf{'subject'} = '【HE】ホームページから資料請求';

#09.送信者に届くメールの件名
$conf{'res_subject'} = 'ヒバライエンジン - 資料請求を受け付けました。';

#10.送信者に届くメールの本文
$conf{'res_body'} = <<'__res_body_eof__';
この度は　hibarai Engine（ヒバライエンジン） ホームページより資料請求をいただきまして、誠にありがとうございます。

フォーム送信後に表示されるページより資料をダウンロードできますが、何かしらのエラーでページが表示されなかった場合、ダウンロードができなかった場合は、
下記URLよりダウンロードができますので、ご確認ください。

■資料ダウンロードURL
https://www.olp.co.jp/hibaraiengine/r_download.html


また、資料請求フォームに質問をご入力頂いている内容につきましては、後ほど担当者より別途連絡をさせていただきます。

資料を閲覧後、ご質問等がある場合は、下記より受け付けております。
